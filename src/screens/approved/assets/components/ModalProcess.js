    ...
    if (dataProcess && dataProcess.length > 0) {
      ...
      if (filter.length > 0) {
        ...
        for (let item of filter) {
          ...
          tmpItem.description = t('approved_assets:person_approved') + item.personApproveName;
          tmpItem.reason =
            (!item.statusID && item.reason)
              ? t('approved_assets:person_approved') + item.reason
              : '';
      ...
      <TouchableOpacity
        ...
        <View style={[cStyles.rounded1, styles.container]}>
          <View style={[
            ...
            <CText styles={'H6 colorWhite'} label={'approved_assets:show_timeline'} />
          </View>
