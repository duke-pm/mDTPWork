...
function CList(props) {
  ...
  const animButtonToTop = useRef(new Animated.Value(cStyles.deviceHeight)).current;
  ...
  const onScroll = (event) => {
    if (!scrollDown.status) {
      ...
      if (scrollDown.offsetY !== 0 && scrollDown.offsetY > event.nativeEvent.contentOffset.y) {
        Animated.timing(
          animButtonToTop,
          {
            toValue: 100,
